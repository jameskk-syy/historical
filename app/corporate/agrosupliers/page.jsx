...
export default function CreateLoan() {
  ...
  const [agronomyserviceproviders,setAgronomyServiceProviders] = useState([]);
  ...
  useEffect(()=>{
    async function fetchProviders(){
      ...
      const  response =  await axios.get(`https://us-central1-farmfuzion.cloudfunctions.net/agronomy_provider?registrationNumber=${ registrationNumber}`);
      setAgronomyServiceProviders(response.data.payload);
    }
    fetchProviders();
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://us-central1-farmfuzion.cloudfunctions.net/agronomy_provider",
        JSON.stringify(agroProvider),
        {
          headers: {
      ...
      console.log("please check your networ");
      setIsLoading(false);
    }
  };
        ...
        <div className="flex flex-grow  flex-col pt-2">
          ...
          <div className="mb-3">
            <nav className="flex">
              ...
              <button
                onClick={() => setActiveStep("agronomyprovider")}
                className={`py-2 px-4 border-b-2 ${
                  activeStep === "agronomyprovider"
                    ? "border-sky-10"
                    : "border-transparent"
                } text-textcolor font-bold rounded-r`}
              >
          ...
          {activeStep === "createagroprovider" && (
            ...
            <div className="flex flex-wrap  md:mx-2 justify-center">
             ...
             <p className="p-2">InsuranceProviders</p>
              {agronomyserviceproviders.map((provider,index) => (
